import java.util.ArrayList;
import java.util.Scanner;

public class mgroth1_assignment2{
    static boolean WordContainsLetter(String word, char letter){
        //if statement for each loop
        char[] wordArray = word.toCharArray();
        for(int i = 0; i < wordArray.length - 1; i++) {
            if(letter == wordArray[i])
            {
                return true;
            }
        }
        return false;
    }

    static boolean wordContainsString(String word, String letters){
        //fix
        if (word.contains(letters)) {
            return true;
        }
        return false;
    }
    static void guessWordWithLetters(English e1, int length, String letters){
        System.out.println("it might be any of these...");
        //loop going through english
        //make english into array list so you can use length and print like an array
        Arraylist<String> eng = e1.words;
        char[] letArray = letters.toCharArray();
        
        for(int i = 0; i < eng.length() -1; i++){
            if (eng.length() == length) {
                boolean match = true;
                for(int j = 0; j < letArray.length - 1; j++){
                    if (!wordContainsString(eng, letArray)) {
                        match = false;
                    }
                }
            }
        }

    }

    static void guessWordWithPattern(English e1, int length, String pattern){
        System.out.println("it might be any of these...");
        //loop
        Arraylist<String> eng = e1.words;

        for(int i = 0; i < eng.length() - 1; i++){
            if(eng.length() == length && wordContainsString(eng, pattern)) {
                System.out.println(eng);
            }
        }
    }


    public static void main(String[] args) {
        Scanner s1 = new Scanner(System.in);
        English e1 = new English();

        //redo
        System.out.println("how many letters are in the word");
        int wordLen = s1.nextInt();

        System.out.println("do you want me to look for letters or a pattern? \n1. Letters \n2. pattern");
        int userChoice = s1.nextInt();

        if(userChoice == 1)
        {
            System.out.println("what letters are in the word?");
            String letters = s1.nextLine();
            GuessWordWithLetters(e1, wordLen, letters);
        } else if (userChoice == 2) {
            System.out.println("What pattern is in the word?");
            String pattern = s1.nextLine();
            GuessWordWithPattern(e1, wordLen, pattern);
        } 


    }
}



class English {
    public ArrayList<String> words = new ArrayList<String>();
  
    public English() {
      words.ensureCapacity(901);
      words.add("a");
      words.add("ability");
      words.add("able");
      words.add("about");
      words.add("above");
      words.add("accept");
      words.add("according");
      words.add("across");
      words.add("act");
      words.add("action");
      words.add("activity");
      words.add("actually");
      words.add("add");
      words.add("administration");
      words.add("adult");
      words.add("affect");
      words.add("after");
      words.add("again");
      words.add("against");
      words.add("age");
      words.add("agency");
      words.add("ago");
      words.add("agree");
      words.add("air");
      words.add("all");
      words.add("allow");
      words.add("almost");
      words.add("along");
      words.add("already");
      words.add("also");
      words.add("although");
      words.add("always");
      words.add("American");
      words.add("among");
      words.add("amount");
      words.add("analysis");
      words.add("and");
      words.add("animal");
      words.add("another");
      words.add("answer");
      words.add("any");
      words.add("anyone");
      words.add("anything");
      words.add("appear");
      words.add("apply");
      words.add("approach");
      words.add("area");
      words.add("argue");
      words.add("arm");
      words.add("around");
      words.add("arrive");
      words.add("art");
      words.add("article");
      words.add("artist");
      words.add("as");
      words.add("ask");
      words.add("at");
      words.add("attack");
      words.add("attention");
      words.add("author");
      words.add("authority");
      words.add("available");
      words.add("avoid");
      words.add("away");
      words.add("baby");
      words.add("back");
      words.add("bad");
      words.add("ball");
      words.add("bank");
      words.add("bar");
      words.add("base");
      words.add("be");
      words.add("beautiful");
      words.add("because");
      words.add("become");
      words.add("bed");
      words.add("before");
      words.add("begin");
      words.add("behavior");
      words.add("behind");
      words.add("believe");
      words.add("benefit");
      words.add("best");
      words.add("better");
      words.add("between");
      words.add("beyond");
      words.add("big");
      words.add("bill");
      words.add("billion");
      words.add("bit");
      words.add("black");
      words.add("blood");
      words.add("blue");
      words.add("board");
      words.add("body");
      words.add("book");
      words.add("both");
      words.add("box");
      words.add("boy");
      words.add("break");
      words.add("bring");
      words.add("brother");
      words.add("build");
      words.add("building");
      words.add("business");
      words.add("but");
      words.add("buy");
      words.add("by");
      words.add("call");
      words.add("camera");
      words.add("campaign");
      words.add("can");
      words.add("candidate");
      words.add("can’t");
      words.add("car");
      words.add("card");
      words.add("care");
      words.add("career");
      words.add("carry");
      words.add("case");
      words.add("catch");
      words.add("cause");
      words.add("cell");
      words.add("center");
      words.add("central");
      words.add("century");
      words.add("certain");
      words.add("certainly");
      words.add("chair");
      words.add("challenge");
      words.add("chance");
      words.add("change");
      words.add("character");
      words.add("charge");
      words.add("check");
      words.add("child");
      words.add("choice");
      words.add("choose");
      words.add("church");
      words.add("city");
      words.add("claim");
      words.add("class");
      words.add("clear");
      words.add("clearly");
      words.add("close");
      words.add("cold");
      words.add("college");
      words.add("color");
      words.add("come");
      words.add("common");
      words.add("community");
      words.add("company");
      words.add("compare");
      words.add("computer");
      words.add("concern");
      words.add("condition");
      words.add("conference");
      words.add("Congress");
      words.add("consider");
      words.add("contain");
      words.add("continue");
      words.add("control");
      words.add("cost");
      words.add("could");
      words.add("country");
      words.add("couple");
      words.add("course");
      words.add("court");
      words.add("cover");
      words.add("create");
      words.add("crime");
      words.add("cultural");
      words.add("culture");
      words.add("cup");
      words.add("current");
      words.add("cut");
      words.add("dark");
      words.add("data");
      words.add("daughter");
      words.add("day");
      words.add("dead");
      words.add("deal");
      words.add("death");
      words.add("decade");
      words.add("decide");
      words.add("decision");
      words.add("deep");
      words.add("defense");
      words.add("degree");
      words.add("Democrat");
      words.add("democratic");
      words.add("describe");
      words.add("design");
      words.add("despite");
      words.add("determine");
      words.add("develop");
      words.add("development");
      words.add("die");
      words.add("difference");
      words.add("different");
      words.add("difficult");
      words.add("direction");
      words.add("director");
      words.add("discover");
      words.add("discuss");
      words.add("disease");
      words.add("do");
      words.add("doctor");
      words.add("dog");
      words.add("door");
      words.add("down");
      words.add("draw");
      words.add("dream");
      words.add("drive");
      words.add("drop");
      words.add("drug");
      words.add("during");
      words.add("each");
      words.add("early");
      words.add("east");
      words.add("easy");
      words.add("eat");
      words.add("economic");
      words.add("economy");
      words.add("edge");
      words.add("education");
      words.add("effect");
      words.add("effort");
      words.add("eight");
      words.add("either");
      words.add("election");
      words.add("else");
      words.add("employee");
      words.add("end");
      words.add("energy");
      words.add("enjoy");
      words.add("enough");
      words.add("enter");
      words.add("entire");
      words.add("environment");
      words.add("environmental");
      words.add("especially");
      words.add("establish");
      words.add("even");
      words.add("evening");
      words.add("event");
      words.add("ever");
      words.add("every");
      words.add("everybody");
      words.add("everyone");
      words.add("everything");
      words.add("evidence");
      words.add("exactly");
      words.add("example");
      words.add("executive");
      words.add("expect");
      words.add("experience");
      words.add("expert");
      words.add("explain");
      words.add("eye");
      words.add("face");
      words.add("fact");
      words.add("factor");
      words.add("fail");
      words.add("fall");
      words.add("family");
      words.add("far");
      words.add("father");
      words.add("fear");
      words.add("federal");
      words.add("feel");
      words.add("feeling");
      words.add("few");
      words.add("field");
      words.add("fight");
      words.add("figure");
      words.add("fill");
      words.add("film");
      words.add("final");
      words.add("finally");
      words.add("financial");
      words.add("find");
      words.add("fine");
      words.add("finish");
      words.add("fire");
      words.add("firm");
      words.add("first");
      words.add("fish");
      words.add("five");
      words.add("floor");
      words.add("fly");
      words.add("focus");
      words.add("follow");
      words.add("food");
      words.add("foot");
      words.add("for");
      words.add("force");
      words.add("foreign");
      words.add("forget");
      words.add("form");
      words.add("former");
      words.add("forward");
      words.add("four");
      words.add("free");
      words.add("friend");
      words.add("from");
      words.add("front");
      words.add("full");
      words.add("fund");
      words.add("future");
      words.add("game");
      words.add("general");
      words.add("get");
      words.add("girl");
      words.add("give");
      words.add("glass");
      words.add("go");
      words.add("goal");
      words.add("good");
      words.add("government");
      words.add("great");
      words.add("green");
      words.add("ground");
      words.add("group");
      words.add("grow");
      words.add("growth");
      words.add("guess");
      words.add("gun");
      words.add("guy");
      words.add("hair");
      words.add("half");
      words.add("hand");
      words.add("hang");
      words.add("happen");
      words.add("happy");
      words.add("hard");
      words.add("have");
      words.add("he");
      words.add("head");
      words.add("health");
      words.add("hear");
      words.add("heart");
      words.add("help");
      words.add("her");
      words.add("here");
      words.add("herself");
      words.add("high");
      words.add("him");
      words.add("himself");
      words.add("his");
      words.add("history");
      words.add("hit");
      words.add("hold");
      words.add("home");
      words.add("hope");
      words.add("hospital");
      words.add("hot");
      words.add("hour");
      words.add("house");
      words.add("how");
      words.add("however");
      words.add("huge");
      words.add("human");
      words.add("hundred");
      words.add("husband");
      words.add("I");
      words.add("idea");
      words.add("identify");
      words.add("if");
      words.add("image");
      words.add("imagine");
      words.add("impact");
      words.add("important");
      words.add("improve");
      words.add("in");
      words.add("include");
      words.add("including");
      words.add("increase");
      words.add("indeed");
      words.add("indicate");
      words.add("individual");
      words.add("industry");
      words.add("information");
      words.add("inside");
      words.add("instead");
      words.add("institution");
      words.add("interest");
      words.add("international");
      words.add("interview");
      words.add("into");
      words.add("involve");
      words.add("issue");
      words.add("it");
      words.add("its");
      words.add("itself");
      words.add("job");
      words.add("join");
      words.add("just");
      words.add("keep");
      words.add("kid");
      words.add("kill");
      words.add("kind");
      words.add("know");
      words.add("knowledge");
      words.add("land");
      words.add("language");
      words.add("large");
      words.add("last");
      words.add("late");
      words.add("later");
      words.add("laugh");
      words.add("law");
      words.add("lawyer");
      words.add("lay");
      words.add("lead");
      words.add("leader");
      words.add("learn");
      words.add("least");
      words.add("leave");
      words.add("left");
      words.add("leg");
      words.add("legal");
      words.add("less");
      words.add("let");
      words.add("letter");
      words.add("level");
      words.add("lie");
      words.add("life");
      words.add("light");
      words.add("like");
      words.add("likely");
      words.add("line");
      words.add("list");
      words.add("listen");
      words.add("little");
      words.add("live");
      words.add("local");
      words.add("long");
      words.add("look");
      words.add("lose");
      words.add("loss");
      words.add("lot");
      words.add("love");
      words.add("low");
      words.add("main");
      words.add("maintain");
      words.add("major");
      words.add("make");
      words.add("man");
      words.add("manage");
      words.add("management");
      words.add("manager");
      words.add("many");
      words.add("market");
      words.add("material");
      words.add("matter");
      words.add("may");
      words.add("maybe");
      words.add("me");
      words.add("mean");
      words.add("measure");
      words.add("media");
      words.add("medical");
      words.add("meet");
      words.add("meeting");
      words.add("member");
      words.add("memory");
      words.add("mention");
      words.add("message");
      words.add("middle");
      words.add("might");
      words.add("military");
      words.add("million");
      words.add("mind");
      words.add("minute");
      words.add("miss");
      words.add("model");
      words.add("moment");
      words.add("money");
      words.add("month");
      words.add("more");
      words.add("morning");
      words.add("most");
      words.add("mother");
      words.add("move");
      words.add("movement");
      words.add("movie");
      words.add("Mr.");
      words.add("much");
      words.add("music");
      words.add("must");
      words.add("my");
      words.add("myself");
      words.add("name");
      words.add("nation");
      words.add("national");
      words.add("natural");
      words.add("nature");
      words.add("near");
      words.add("nearly");
      words.add("necessary");
      words.add("need");
      words.add("network");
      words.add("never");
      words.add("new");
      words.add("news");
      words.add("next");
      words.add("nice");
      words.add("night");
      words.add("no");
      words.add("north");
      words.add("not");
      words.add("note");
      words.add("nothing");
      words.add("now");
      words.add("number");
      words.add("occur");
      words.add("of");
      words.add("off");
      words.add("offer");
      words.add("office");
      words.add("officer");
      words.add("official");
      words.add("often");
      words.add("oh");
      words.add("oil");
      words.add("ok");
      words.add("old");
      words.add("on");
      words.add("once");
      words.add("one");
      words.add("only");
      words.add("onto");
      words.add("open");
      words.add("operation");
      words.add("opportunity");
      words.add("or");
      words.add("order");
      words.add("organization");
      words.add("other");
      words.add("others");
      words.add("our");
      words.add("out");
      words.add("outside");
      words.add("over");
      words.add("own");
      words.add("page");
      words.add("pain");
      words.add("paper");
      words.add("parent");
      words.add("part");
      words.add("particular");
      words.add("particularly");
      words.add("party");
      words.add("pass");
      words.add("past");
      words.add("patient");
      words.add("pay");
      words.add("peace");
      words.add("people");
      words.add("per");
      words.add("perform");
      words.add("performance");
      words.add("perhaps");
      words.add("period");
      words.add("person");
      words.add("personal");
      words.add("phone");
      words.add("physical");
      words.add("pick");
      words.add("picture");
      words.add("piece");
      words.add("place");
      words.add("plan");
      words.add("plant");
      words.add("play");
      words.add("player");
      words.add("PM");
      words.add("point");
      words.add("police");
      words.add("policy");
      words.add("political");
      words.add("politics");
      words.add("poor");
      words.add("popular");
      words.add("population");
      words.add("position");
      words.add("possible");
      words.add("power");
      words.add("practice");
      words.add("prepare");
      words.add("present");
      words.add("president");
      words.add("pressure");
      words.add("pretty");
      words.add("price");
      words.add("private");
      words.add("probably");
      words.add("problem");
      words.add("process");
      words.add("produce");
      words.add("product");
      words.add("production");
      words.add("professor");
      words.add("program");
      words.add("project");
      words.add("property");
      words.add("protect");
      words.add("prove");
      words.add("provide");
      words.add("public");
      words.add("pull");
      words.add("push");
      words.add("put");
      words.add("quality");
      words.add("question");
      words.add("quickly");
      words.add("quite");
      words.add("race");
      words.add("radio");
      words.add("raise");
      words.add("range");
      words.add("rate");
      words.add("rather");
      words.add("reach");
      words.add("read");
      words.add("ready");
      words.add("real");
      words.add("realize");
      words.add("really");
      words.add("reason");
      words.add("receive");
      words.add("recent");
      words.add("recently");
      words.add("recognize");
      words.add("record");
      words.add("red");
      words.add("reduce");
      words.add("region");
      words.add("relationship");
      words.add("religious");
      words.add("remain");
      words.add("remember");
      words.add("remove");
      words.add("report");
      words.add("represent");
      words.add("Republican");
      words.add("require");
      words.add("research");
      words.add("resource");
      words.add("respond");
      words.add("response");
      words.add("rest");
      words.add("result");
      words.add("return");
      words.add("reveal");
      words.add("right");
      words.add("rise");
      words.add("risk");
      words.add("road");
      words.add("rock");
      words.add("role");
      words.add("room");
      words.add("rule");
      words.add("run");
      words.add("same");
      words.add("save");
      words.add("say");
      words.add("scene");
      words.add("school");
      words.add("science");
      words.add("sea");
      words.add("season");
      words.add("seat");
      words.add("second");
      words.add("section");
      words.add("security");
      words.add("see");
      words.add("seek");
      words.add("seem");
      words.add("sell");
      words.add("send");
      words.add("sense");
      words.add("series");
      words.add("serious");
      words.add("serve");
      words.add("service");
      words.add("set");
      words.add("seven");
      words.add("several");
      words.add("sex");
      words.add("shake");
      words.add("share");
      words.add("she");
      words.add("shoot");
      words.add("short");
      words.add("should");
      words.add("shoulder");
      words.add("show");
      words.add("side");
      words.add("sign");
      words.add("significant");
      words.add("similar");
      words.add("simple");
      words.add("simply");
      words.add("since");
      words.add("single");
      words.add("sister");
      words.add("sit");
      words.add("site");
      words.add("situation");
      words.add("six");
      words.add("size");
      words.add("skill");
      words.add("small");
      words.add("so");
      words.add("social");
      words.add("society");
      words.add("some");
      words.add("someone");
      words.add("something");
      words.add("sometimes");
      words.add("son");
      words.add("song");
      words.add("soon");
      words.add("sort");
      words.add("sound");
      words.add("source");
      words.add("south");
      words.add("space");
      words.add("speak");
      words.add("special");
      words.add("specific");
      words.add("spend");
      words.add("sport");
      words.add("spring");
      words.add("staff");
      words.add("stage");
      words.add("stand");
      words.add("standard");
      words.add("star");
      words.add("start");
      words.add("state");
      words.add("statement");
      words.add("station");
      words.add("stay");
      words.add("step");
      words.add("still");
      words.add("stock");
      words.add("stop");
      words.add("store");
      words.add("story");
      words.add("strategy");
      words.add("street");
      words.add("strong");
      words.add("structure");
      words.add("student");
      words.add("study");
      words.add("stuff");
      words.add("style");
      words.add("subject");
      words.add("success");
      words.add("such");
      words.add("suddenly");
      words.add("suggest");
      words.add("summer");
      words.add("support");
      words.add("sure");
      words.add("system");
      words.add("table");
      words.add("take");
      words.add("talk");
      words.add("tax");
      words.add("teach");
      words.add("teacher");
      words.add("team");
      words.add("technology");
      words.add("television");
      words.add("tell");
      words.add("ten");
      words.add("term");
      words.add("test");
      words.add("than");
      words.add("thank");
      words.add("that");
      words.add("their");
      words.add("them");
      words.add("themselves");
      words.add("then");
      words.add("theory");
      words.add("there");
      words.add("these");
      words.add("they");
      words.add("thing");
      words.add("think");
      words.add("third");
      words.add("this");
      words.add("those");
      words.add("though");
      words.add("thought");
      words.add("thousand");
      words.add("three");
      words.add("through");
      words.add("throughout");
      words.add("throw");
      words.add("thus");
      words.add("time");
      words.add("to");
      words.add("today");
      words.add("together");
      words.add("tonight");
      words.add("too");
      words.add("top");
      words.add("toward");
      words.add("town");
      words.add("trade");
      words.add("traditional");
      words.add("training");
      words.add("treat");
      words.add("treatment");
      words.add("tree");
      words.add("trial");
      words.add("trip");
      words.add("trouble");
      words.add("true");
      words.add("truth");
      words.add("try");
      words.add("turn");
      words.add("TV");
      words.add("two");
      words.add("type");
      words.add("under");
      words.add("understand");
      words.add("unit");
      words.add("until");
      words.add("up");
      words.add("upon");
      words.add("us");
      words.add("use");
      words.add("usually");
      words.add("value");
      words.add("various");
      words.add("very");
      words.add("view");
      words.add("visit");
      words.add("voice");
      words.add("wait");
      words.add("walk");
      words.add("wall");
      words.add("want");
      words.add("war");
      words.add("watch");
      words.add("water");
      words.add("way");
      words.add("we");
      words.add("weapon");
      words.add("wear");
      words.add("week");
      words.add("weight");
      words.add("well");
      words.add("west");
      words.add("what");
      words.add("whatever");
      words.add("when");
      words.add("where");
      words.add("whether");
      words.add("which");
      words.add("while");
      words.add("white");
      words.add("who");
      words.add("whole");
      words.add("whose");
      words.add("why");
      words.add("wide");
      words.add("wife");
      words.add("will");
      words.add("win");
      words.add("window");
      words.add("with");
      words.add("within");
      words.add("without");
      words.add("woman");
      words.add("wonder");
      words.add("won’t");
      words.add("word");
      words.add("work");
      words.add("worker");
      words.add("world");
      words.add("worry");
      words.add("would");
      words.add("write");
      words.add("writer");
      words.add("wrong");
      words.add("yeah");
      words.add("year");
      words.add("yes");
      words.add("yet");
      words.add("you");
      words.add("young");
      words.add("your");
      words.add("yourself");
    }
  }